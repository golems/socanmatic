...
static void init( struct can402_cx *cx ) {
    ...
    for( size_t i = 0; i < cx->drive_set.n; i ++ ) {
        ...
        // DEBUG***************************************************************************
        // Init mainly gets the info from the drives, so we can read it directly from
        uint16_t ctrl_word; uint16_t status_word; uint32_t abort_code;
        r = canmat_402_ul_controlword( cx->drive_set.cif,
        			cx->drive_set.drive[i].node_id,
					&ctrl_word,
					&abort_code );

        if( r != CANMAT_OK ) { printf("Something wrong when retrieving the control word of the drive \n");}
        printf("[DIAGNOSTIC] Control word: 0x%x \n", ctrl_word );

        r = canmat_402_ul_statusword( cx->drive_set.cif,
        			cx->drive_set.drive[i].node_id,
					&status_word,
					&abort_code );

        if( r != CANMAT_OK ) { printf("Something wrong when retrieving the status word of the drive \n");}
        printf("[DIAGNOSTIC] Status word: 0x%x \n", ctrl_word );
